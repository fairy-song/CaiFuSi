...
> 评审日期：2026-09-01
> 评审范围：XiaoCow666/Caifusi `main` 分支（含 MySQL 持久化提交）+ 本地最新代码（前端 + 后端 + 移动壳 + 文档）
...
| 数据存储 | 三引擎存储抽象（`DB_TYPE` 切换）：内存 memory / MySQL（pymysql + schema.sql 建表）/ Firebase Firestore |
...
- 配置集中：[config.py](backend/app/config.py) 全部从环境变量读取（`SECRET_KEY`、`ZHIPUAI_API_KEY`、`GEMINI_API_KEY`、`DB_TYPE`、MySQL 连接参数、Firebase SDK 路径），仓库内无硬编码密钥。
...
- **存储抽象**：`UserDataService` 统一封装用户数据（评估、目标、教练消息等）的存取，由 `DB_TYPE` 环境变量在内存 / MySQL / Firestore 三种引擎间切换；MySQL 模式由 `db_mysql.py`（pymysql、utf8mb4、DictCursor）实现，`schema.sql` 负责建表与初始化。
- **AI 教练链路**（核心链路）：
...
```
用户填评测问卷 → assessment_routes 打分+建议 → 按 DB_TYPE 写入存储引擎(内存/MySQL/Firestore)
                                              ↓
...
7. **存储层抽象**：`DB_TYPE` 三引擎切换（内存 / MySQL / Firestore）与统一数据服务封装，为从演示走向真实持久化预留了平滑路径。
